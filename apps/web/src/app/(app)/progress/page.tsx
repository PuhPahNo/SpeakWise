...
export default async function ProgressPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <h1 className="font-display text-3xl mb-6">Your progress</h1>

      <div className="grid sm:grid-cols-3 gap-3 mb-8">
        <Stat label="XP earned" value={xp.toString()} />
        <Stat label="Skills active" value={dashboard.skills.length.toString()} />
        <Stat
          label="Vocabulary mastered"
          value={String(dashboard.vocabularyCounts.mastered ?? 0)}
        />
      </div>

      <h2 className="font-display text-xl mb-3">Skill graph</h2>
        ...
        <ul className="space-y-2">
          {dashboard.skills.map((s) => (
            <li key={s.slug} className="flex items-center gap-3">
              ...
              <div className="h-1.5 bg-ink-100 rounded-full overflow-hidden w-32">
