...
> Status: working web application with learner, tutor, curriculum, voice, memory,
> and admin flows. The product specifications and build history remain in this repo.
...
```
apps/
  web/        Next.js 15 (App Router) — UI + REST API + scheduled jobs
...
.github/      CI workflow
render.yaml   Render Blueprint — Postgres + one web service
...
- **ORM:** Prisma
- **Auth:** In-house username/password provisioning + encrypted, revocable session cookies
- **AI:** OpenAI (gpt-4o + gpt-4o-mini, whisper-1, text-embedding-3-small) + ElevenLabs TTS
- **Hosting:** Render (web service + Postgres; guarded scheduled jobs run in-process)
...
# → http://localhost:3001  →  sign in with the username + password from step 5
```
...
phone, point the phone at your laptop's local IP on port 3001.
...
| Service     | Why                                                    | Where                                               |
| ----------- | ------------------------------------------------------ | --------------------------------------------------- |
| Render      | Postgres + web service                                 | https://dashboard.render.com                        |
...
2. In Render → New → Blueprint → connect this repo. Render reads `render.yaml`
   and creates the Postgres database and web service.
3. Render will prompt for any env var marked `sync: false`: `OPENAI_API_KEY`,
   `ELEVENLABS_API_KEY`, and `APP_URL` /
...
4. Every deploy runs the versioned migration and idempotent seed in Render's
   pre-deploy phase. Existing pre-migration databases are baselined automatically.
5. Create your first admin user from the Render shell:
...
| 04 Curriculum & Skill Graph     | `packages/db/src/seeds/curriculum/` + `services/curriculum/`                |
...
| 12 Tutor Mode                   | `apps/web/src/server/services/classroom/` + `app/(tutor)/classroom/`        |
